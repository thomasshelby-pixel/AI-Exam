...
export const LegalDocumentView: React.FC<LegalDocumentViewProps> = ({ activeDocType }) => {
  ...
  return (
    <div id="legal-document-view" className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-100 py-8 sm:py-12 px-4 sm:px-6 lg:px-8 transition-colors duration-150">
      <div className="max-w-4xl mx-auto space-y-6 sm:space-y-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-5">
          <div>
            <div className="flex items-center gap-2 text-xs text-blue-700 dark:text-blue-400 mb-1">
              <span className="hover:text-blue-900 dark:hover:text-blue-300 cursor-pointer font-medium" onClick={() => navigate('/')}>
                Home
              </span>
              <span className="text-slate-400">/</span>
              <span className="font-bold text-slate-900 dark:text-white">Legal & Policies</span>
            </div>
          ...
          <div className="flex items-center gap-2 print:hidden">
            <button
              ...
              className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-xs font-semibold text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-700 transition shadow-xs cursor-pointer"
            >
              ...
              <button
                ...
                className={`px-4 py-2.5 rounded-xl text-xs font-bold transition flex items-center gap-2 cursor-pointer ${
                  isActive
                    ...
                    : 'bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-slate-50 dark:hover:bg-slate-800'
                }`}
              >
        ...
        <div className="p-4 sm:p-5 rounded-xl bg-blue-50/60 dark:bg-blue-950/40 border border-blue-200 dark:border-blue-900/50 text-slate-800 dark:text-slate-200 space-y-2">
        ...
        {loading ? (
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-12 text-center shadow-xs">
            ...
            <p className="text-xs font-semibold text-slate-700 dark:text-slate-300">Retrieving official published document...</p>
          </div>
        ) : error ? (
          <div className="bg-rose-50 dark:bg-rose-950/40 border border-rose-200 dark:border-rose-800 p-6 rounded-xl text-rose-800 dark:text-rose-300 text-xs">
            {error}
          </div>
        ) : document ? (
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xs overflow-hidden">
            {/* Document Header Metadata */}
            <div className="p-6 sm:p-8 border-b border-slate-200 dark:border-slate-800 bg-slate-50/80 dark:bg-slate-800/40">
              <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
                <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[11px] font-bold bg-blue-100 dark:bg-blue-900/40 text-blue-800 dark:text-blue-300 border border-blue-200 dark:border-blue-800">
                  <CheckCircle2 className="w-3.5 h-3.5 text-blue-700 dark:text-blue-400" />
                  Official Version {document.version} (Active)
                </span>
                <span className="text-xs text-slate-600 dark:text-slate-400 font-medium">
                  Effective: <strong className="font-bold text-slate-900 dark:text-white">{document.effective_date}</strong>
                  <span className="mx-2 text-slate-400">|</span>
                  Last Updated: <strong className="font-bold text-slate-900 dark:text-white">{document.last_updated_date}</strong>
                </span>
              </div>
            ...
            <div className="p-6 sm:p-8 space-y-4 font-sans text-slate-700 dark:text-slate-300">
              {renderDocumentContent(document.content)}
            </div>

            {/* Contact & Support Section */}
            <div className="p-6 sm:p-8 border-t border-slate-200 dark:border-slate-800 bg-slate-50/80 dark:bg-slate-800/40 space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-1">
                {/* Email Support */}
                <div className="p-4 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-xs flex items-center gap-3.5">
                  <div className="w-10 h-10 rounded-lg bg-blue-50 dark:bg-blue-900/30 border border-blue-100 dark:border-blue-800 flex items-center justify-center shrink-0 text-blue-600 dark:text-blue-400">
                ...
                {/* Instagram Channel */}
                <div className="p-4 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-xs flex items-center gap-3.5">
                  <div className="w-10 h-10 rounded-lg bg-pink-50 dark:bg-pink-950/30 border border-pink-100 dark:border-pink-800 flex items-center justify-center shrink-0 text-pink-600 dark:text-pink-400">
                    <Instagram className="w-5 h-5" />
                  </div>
                  <div className="min-w-0">
                    <p className="text-[11px] font-bold text-slate-900 dark:text-white uppercase tracking-wider">Instagram</p>
                    <a
                      href={settings?.instagram_url || 'https://insta.openinapp.co/utw2r'}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex items-center gap-1.5 text-xs sm:text-sm font-semibold text-pink-600 dark:text-pink-400 hover:underline"
                    >
                      <span>Instagram</span>
                    </a>
                  </div>
        ...
        <div className="pt-4 flex flex-wrap items-center justify-between gap-4 text-xs text-slate-600 dark:text-slate-400 border-t border-slate-200 dark:border-slate-800 print:hidden">
          <div className="flex items-center gap-3 sm:gap-4">
            <Link to="/terms" className="hover:text-blue-600 dark:hover:text-blue-400 font-medium transition">Terms of Service</Link>
            <span className="text-slate-300 dark:text-slate-600">&bull;</span>
            <Link to="/privacy-policy" className="hover:text-blue-600 dark:hover:text-blue-400 font-medium transition">Privacy Policy</Link>
            <span className="text-slate-300 dark:text-slate-600">&bull;</span>
            <Link to="/refund-policy" className="hover:text-blue-600 dark:hover:text-blue-400 font-medium transition">Refund & Cancellation</Link>
          </div>
          <p className="text-slate-500 dark:text-slate-400">&copy; 2026 CA Exam Checker AI. All rights reserved.</p>
        </div>
      </div>
    </div>
  );
...
/**
 * Parses and formats legal document content to enforce clean visual hierarchy:
 * - Section headings in red/rose
 * - Body / list content in accessible text
 * - Important terms / values in high-contrast bold
    ...
    <div className="space-y-4">
      {blocks.map((block, idx) => {
        ...
        if (headingMatch) {
          ...
          return (
            <div key={idx} className="pt-3 first:pt-0">
              <h3 className="text-red-700 dark:text-rose-400 font-bold text-base sm:text-lg tracking-tight mb-2 pb-1 border-b border-red-100/60 dark:border-rose-900/40">
                {headingText}
              </h3>
        ...
        if (
          ...
          return (
            <h3 key={idx} className="text-red-700 dark:text-rose-400 font-bold text-base tracking-tight pt-3 mb-1">
              {trimmed}
            </h3>
          );
        }
    ...
    <div className="space-y-2">
      {lines.map((line, lIdx) => {
        ...
        if (lTrimmed.startsWith('- ') || lTrimmed.startsWith('* ')) {
          return (
            <li key={lIdx} className="text-blue-900 dark:text-slate-200 text-xs sm:text-sm leading-relaxed list-disc list-inside ml-2">
        ...
        if (lTrimmed.toLowerCase().includes('instagram')) {
          return (
            <p key={lIdx} className="text-blue-900 dark:text-slate-200 text-xs sm:text-sm leading-relaxed flex items-center gap-1.5 flex-wrap">
              <span className="font-bold text-blue-950 dark:text-white">Instagram:</span>
              <a
        ...
        return (
          <p key={lIdx} className="text-blue-900 dark:text-slate-200 text-xs sm:text-sm leading-relaxed">
...
/**
 * Highlights critical regulatory, contractual, and technical terms in BOLD.
 */
