...
import { ModelAnswerComparisonView } from '../../components/student/ModelAnswerComparisonView.js';
import { ModelAnswerComparisonModal } from '../../components/student/ModelAnswerComparisonModal.js';
...
export const EvaluationReportView: React.FC<EvaluationReportViewProps> = ({
  ...
  const [comparisonTargetQuestionIdx, setComparisonTargetQuestionIdx] = useState<number>(0);
  const [breakdownViewMode, setBreakdownViewMode] = useState<'AUDIT' | 'COMPARISON'>('AUDIT');
    ...
    <div className="max-w-5xl mx-auto px-4 py-6 text-slate-800 dark:text-slate-100 space-y-6 print:p-0 print:text-black">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 print:hidden">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {/* 3.5. Model vs. Submitted Answer Comparison */}
          <button
            id="open-model-comparison-btn"
            onClick={() => {
              setComparisonTargetQuestionIdx(0);
              setIsComparisonModalOpen(true);
            }}
            className="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold transition shadow-sm cursor-pointer"
            title="Overlay Model Answer versus Submitted Answer side-by-side to highlight exact points of divergence"
          >
            <Columns className="w-3.5 h-3.5" />
            <span>Model vs. Submitted</span>
          </button>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-5 space-y-5 shadow-sm print:border-gray-300 print:bg-white print:text-black">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2.5 print:hidden">
            <div
              id="breakdown-view-mode-selector"
            ...
            >
              <button
                type="button"
                id="breakdown-view-audit-btn"
                onClick={() => setBreakdownViewMode('AUDIT')}
                className={`px-3 py-1 font-bold rounded-lg transition-colors cursor-pointer flex items-center gap-1.5 ${
                  breakdownViewMode === 'AUDIT'
                    ? 'bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-2xs'
                    : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'
                }`}
              >
                <span>Audit Cards</span>
              </button>
              <button
                type="button"
                id="breakdown-view-comparison-btn"
                onClick={() => setBreakdownViewMode('COMPARISON')}
                className={`px-3 py-1 font-bold rounded-lg transition-colors cursor-pointer flex items-center gap-1.5 ${
                  breakdownViewMode === 'COMPARISON'
                    ? 'bg-blue-600 text-white shadow-2xs'
                    : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'
                }`}
                title="Switch to Side-by-Side Model Answer vs. Submitted Answer comparison"
              >
                <Columns className="w-3.5 h-3.5" />
        ...
        {breakdownViewMode === 'COMPARISON' ? (
          <ModelAnswerComparisonView
            ...
            initialQuestionIndex={comparisonTargetQuestionIdx}
          />
        ) : (
          /* Questions List */
              ...
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                  ...
                  <div className="flex items-center gap-2.5 flex-wrap">
                    ...
                    {/* Compare Model Answer Button */}
                    <button
                      type="button"
                      id={`compare-q-${q.questionNumber}-btn`}
                      onClick={() => {
                        setComparisonTargetQuestionIdx(idx);
                        setIsComparisonModalOpen(true);
                      }}
                      className="print:hidden inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-blue-50 dark:bg-blue-950/60 border border-blue-200 dark:border-blue-800 hover:bg-blue-100 dark:hover:bg-blue-900/60 text-[11px] font-bold text-blue-700 dark:text-blue-300 transition shadow-2xs cursor-pointer"
                      title={`Overlay Model Answer versus Submitted Answer for Question ${q.questionNumber}`}
                    >
                      <Columns className="w-3 h-3 text-blue-600 dark:text-blue-400" />
                      <span>Compare Model Answer</span>
                    </button>
      ...
      {/* Model Answer vs. Submitted Answer Side-by-Side Comparison Modal */}
      <ModelAnswerComparisonModal
