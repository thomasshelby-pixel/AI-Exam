...
export async function generateDetailedReportPdf(
  ...
  const totalAwarded = evalData.totalMarks ?? resultJson?.totalMarksAwarded ?? 0;
  const maxMarks = evalData.maximumMarks ?? resultJson?.maximumMarks ?? 100;
  const percentage = maxMarks > 0 ? (totalAwarded / maxMarks) * 100 : 0;
  const resultStatus = percentage >= 60 ? 'EXEMPTION' : percentage >= 40 ? 'PASS' : 'FAIL';
  ...
  safeDrawText(page, 'COMPREHENSIVE EVALUATION & STEP-MARKING REPORT', {
    x: 40,
    y: height - 62,
    size: 10,
    font: helveticaBold,
  ...
  safeDrawText(page, `Report ID: ${evalData.id}`, {
    x: 40,
    y: height - 80,
    size: 8.5,
    font: helvetica,
  ...
  safeDrawText(
    page,
    'Academic Benchmark Notice: Independent evaluation aligned with ICAI patterns. Not endorsed by or affiliated with ICAI.',
    {
      x: 45,
      y: height - 124,
      size: 7.5,
  ...
  safeDrawText(page, `Candidate: ${evalData.studentName || 'Student'} (${evalData.icaiRegistrationNumber || 'N/A'})`, {
    x: 50,
    y: height - 165,
    size: 11,
    font: helveticaBold,
  ...
  safeDrawText(page, `Course Level: ${evalData.level}  |  Attempt: ${evalData.attempt || 'May 2026'}`, {
    x: 50,
    y: height - 185,
    size: 9.5,
    font: helvetica,
  ...
  safeDrawText(page, `Subject: ${evalData.subjectName} (${evalData.paper || 'Paper 1'})`, {
    x: 50,
    y: height - 205,
    size: 9.5,
    font: helveticaBold,
  ...
  // Total Score display
  safeDrawText(page, `SCORE: ${totalAwarded.toFixed(1)} / ${maxMarks} (${percentage.toFixed(1)}%)`, {
    x: width - 220,
    y: height - 170,
    size: 12,
    font: helveticaBold,
  ...
  safeDrawText(page, `RESULT: ${resultStatus}`, {
    x: width - 220,
    y: height - 190,
    size: 11,
    font: helveticaBold,
    color: resultStatus === 'FAIL' ? rgb(0.8, 0.1, 0.1) : rgb(0.1, 0.6, 0.2),
  });

  // Strengths & Areas to Improve
  const strengths: string[] = resultJson?.keyStrengths || [
    'Good compliance with statutory standard provisions',
    'Clear step workings in compulsory questions',
  ];
  const improvements: string[] = resultJson?.areasForImprovement || [
    'Include complete working notes for calculation steps',
    'Improve speed management in long descriptive answers',
  ];

  safeDrawText(page, 'KEY STRENGTHS', { x: 40, y: height - 260, size: 10, font: helveticaBold, color: blueNavy });
  strengths.slice(0, 3).forEach((st, idx) => {
    safeDrawText(page, `- ${st.substring(0, 90)}`, { x: 45, y: height - 278 - idx * 16, size: 8.5, font: helvetica, color: darkSlate });
  });

  safeDrawText(page, 'AREAS FOR IMPROVEMENT', { x: 40, y: height - 340, size: 10, font: helveticaBold, color: rgb(0.8, 0.2, 0.2) });
  improvements.slice(0, 3).forEach((imp, idx) => {
    safeDrawText(page, `- ${imp.substring(0, 90)}`, { x: 45, y: height - 358 - idx * 16, size: 8.5, font: helvetica, color: darkSlate });
  });

  // Question list
  const questions = resultJson?.questionWiseBreakdown || [];
  safeDrawText(page, 'QUESTION-WISE BREAKDOWN', { x: 40, y: height - 420, size: 10, font: helveticaBold, color: blueNavy });

  let qY = height - 445;
  questions.slice(0, 7).forEach((q: any) => {
    page.drawRectangle({
      x: 35,
      y: qY - 30,
      width: width - 70,
    ...
    safeDrawText(page, `Question ${q.questionNumber}: ${q.marksAwarded} / ${q.maxMarks} Marks`, {
      x: 45,
      y: qY - 12,
      size: 9,
      font: helveticaBold,
    ...
    const rem = (q.examinerRemarks || q.feedback || '').substring(0, 85);
    safeDrawText(page, rem, {
      x: 45,
      y: qY - 24,
      size: 7.5,
      font: helvetica,
  ...
  safeDrawText(page, 'Digitally Generated and Evaluated by CA Exam Checker AI Engine', {
    x: 40,
    y: 40,
    size: 8,
    font: helveticaBold,
  ...
  safeDrawText(page, `Generated on ${new Date().toLocaleString('en-IN', { timeZone: 'Asia/Kolkata' })} IST`, {
    x: 40,
    y: 28,
    size: 7.5,
    font: helvetica,
  ...
  const reportBytes = await pdfDoc.save();
  return Buffer.from(reportBytes);
}

/**
 * Generates an Authentic ICAI Examination Candidate Answer Booklet PDF Archive
 * when downloading original answer sheets.
 */
